...
class SpeedSkein:
	...
	def getFlowRateString(self):
		...
		if self.isBridgeLayer:
			flowRate = self.repository.bridgeFlowRateMultiplier.value * self.repository.bridgeFeedRateMultiplier.value * self.repository.flowRateSetting.value * self.feedRatePerSecond
	...
	def getSpeededLine(self, line, splitLine):
		...
		if self.isBridgeLayer:
			feedRateMinute = self.repository.bridgeFeedRateMultiplier.value * 60.0 * self.feedRatePerSecond
		if self.isPerimeterPath:
