...
Ease is a script to smooth the surface ease of an object by replacing the perimeter surface with a surface printed at half the carve
...
http://adventuresin3-dprinting.blogspot.com/2011/05/easening.html

The ease manual page is at:
http://fabmetheus.crsndoo.com/wiki/index.php/Skeinforge_Ease


==Operation==
The default 'Activate Ease' checkbox is off.  When it is on, the functions described below will work, when it is off, the functions will not be called.
...
Defines which layer of the print the easening process starts from. It is not wise to set this to zero, easening the bottom layer is likely to cause the bottom perimeter not to adhere well to the print surface.

====Tips====
Due to the very small Z-axis moves easening can generate as it prints the perimeter, it can cause the Z-axis speed to be limited by the Limit plug-in, if you have it enabled. This can cause some printers to pause excessively during each layer change. To overcome this, ensure that the Z-axis max speed in the Limit tool is set to an appropriate value for your printer, e.g. 10mm/s

Since Ease prints two half-height perimeter layers for each layer, printing the perimeter last causes the print head to travel down from the current print height. Depending on the shape of your extruder nozzle, you may get higher quality prints if you print the perimeters first, so the print head always travels up.  This is set via the Thread Sequence Choice setting in the Fill tool.


==Examples==
The following examples ease the file Screw Holder Bottom.stl.  The examples are run in a terminal in the folder which contains Screw Holder Bottom.stl and ease.py.

> python ease.py
This brings up the ease dialog.

> python ease.py Screw Holder Bottom.stl
The ease tool is parsing the file:
Screw Holder Bottom.stl
..
The ease tool has created the file:
.. Screw Holder Bottom_ease.gcode
...
def getCraftedText(fileName, gcodeText, repository=None):
	'Ease a gcode linear move text.'
	return getCraftedTextFromText(archive.getTextIfEmpty(fileName, gcodeText), repository)

def getCraftedTextFromText(gcodeText, repository=None):
	'Ease a gcode linear move text.'
	if gcodec.isProcedureDoneOrFileIsEmpty(gcodeText, 'ease'):
		return gcodeText
	if repository == None:
		repository = settings.getReadRepository(EaseRepository())
	if not repository.activateEase.value:
		return gcodeText
	return EaseSkein().getCraftedGcode(gcodeText, repository)

def getNewRepository():
	'Get new repository.'
	return EaseRepository()

def writeOutput(fileName, shouldAnalyze=True):
	'Ease a gcode linear move file.  Chain ease the gcode if it is not already easened.'
	skeinforge_craft.writeChainTextWithNounMessage(fileName, 'ease', shouldAnalyze)


class EaseRepository:
	'A class to handle the ease settings.'
	def __init__(self):
		'Set the default settings, execute title & settings fileName.'
		skeinforge_profile.addListsToCraftTypeRepository('skeinforge_application.skeinforge_plugins.craft_plugins.ease.html', self )
		self.fileNameInput = settings.FileNameInput().getFromFileName( fabmetheus_interpret.getGNUTranslatorGcodeFileTypeTuples(), 'Open File for Ease', self, '')
		self.openWikiManualHelpPage = settings.HelpPage().getOpenFromAbsolute('http://fabmetheus.crsndoo.com/wiki/index.php/Skeinforge_Ease')
		self.activateEase = settings.BooleanSetting().getFromValue('Activate Ease', self, False )
		...
		self.executeTitle = 'Ease'

	def execute(self):
		'Ease button has been clicked.'
...
class EaseSkein:
	'A class to ease a skein of extrusions.'
	def __init__(self):
		'Initialize.'
	...
	def addEasenedPerimeter(self):
		'Add easened perimeter.'
		if self.perimeter == None:
			return
		self.perimeter = self.perimeter[: -1]
		innerPerimeter = intercircle.getLargestInsetLoopFromLoop(self.perimeter, self.quarterPerimeterWidth*0.7853)
		innerPerimeter = self.getClippedSimplifiedLoopPathByLoop(innerPerimeter)
		outerPerimeter = intercircle.getLargestInsetLoopFromLoop(self.perimeter, -self.quarterPerimeterWidth)#		outerPerimeter = intercircle.getLargestInsetLoopFromLoop(self.perimeter, -self.quarterPerimeterWidth)
		outerPerimeter = self.getClippedSimplifiedLoopPathByLoop(outerPerimeter)
		lowerZ = self.oldLocation.z - self.halfLayerThickness
		self.addFlowRateLine(self.oldFlowRate*1.2)
	...
	def getCraftedGcode( self, gcodeText, repository ):
		'Parse gcode text and store the ease gcode.'
	...
	def parseInitialization(self):
		...
		for self.lineIndex in xrange(len(self.lines)):
			...
			if firstWord == '(</extruderInitialization>)':
				self.distanceFeedRate.addLine('(<procedureName> ease </procedureName>)')
				return
	...
	def parseLine(self, line):
		'Parse a gcode line and add it to the ease skein.'
		splitLine = gcodec.getSplitLineBeforeBracketSemicolon(line)
		if len(splitLine) < 1:
			return
		...
		if firstWord == 'G1':
			...
			self.layerCount.printProgressIncrement('ease')
			self.layerIndex += 1
			self.setInfill()
		elif firstWord == '(<loop>':
			self.infill = None
			...
			self.addEasenedPerimeter()
			self.setInfill()
			self.perimeter = None
		if firstWord == 'M103':
			if self.infill != None:
				self.addEasenedInfill()
				self.setInfill()
				return
