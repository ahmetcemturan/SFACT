...
class ChopSkein:
	...
	def addExtraTopLayerIfNecessary( self, carving, extrusionHeight, rotatedLoopLayers ):
		"""Add extra top layer if necessary."""
		topRotatedBoundaryLayer = rotatedLoopLayers[-1]
		cuttingSafeHeight = topRotatedBoundaryLayer.z + 0.5001 * extrusionHeight
		if cuttingSafeHeight > carving.getCarveCornerMaximum().z:
			return
		extraTopRotatedBoundaryLayer = topRotatedBoundaryLayer.getCopyAtZ( topRotatedBoundaryLayer.z + extrusionHeight )
	...
	def getCarvedSVG( self, carving, fileName, repository ):
		...
		if repository.addExtraTopLayerIfNecessary.value:
			self.addExtraTopLayerIfNecessary( carving, extrusionHeight, rotatedLoopLayers )
